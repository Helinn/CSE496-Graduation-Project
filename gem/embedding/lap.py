...
class LaplacianEigenmaps(StaticGraphEmbedding):
    ...
    def learn_embedding(self, graph=None, edge_f=None,
        ...
        self._X = v[:, 1:]
        embedding =[[None for _ in range(2)] for _ in range(len(self._X))]
        for a in range(len(self._X)):
            embedding[a][0] = float(self._X[a][0].real)
            embedding[a][1] = self._X[a][1].real
        ...
        zipbObj = zip(list(graph.nodes), embedding)
